...
def read_log(file, readin)
  ...
  File.foreach(file) do |line|
    ...
    if /Nmap/.match(line_info["%{User-Agent}i"])
      incident_num += 1
      puts incident_num.to_s + ". ALERT: NMAP scan is detected from " + line_info["%h"] + " (HTTP) (" + line_info["%r"] + ")!\n"
    end 
    ...
    if /scripts\/setup.php/.match(line_info["%r"])
      incident_num += 1
      puts incident_num.to_s + ". ALERT: SHELLCODE is detected from " + line_info["%h"] + " (HTTP) (" + line_info["%r"] + ")!\n"
    end
  end
